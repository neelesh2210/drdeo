                            ...
                            <div class="hs-unfold float-right col-6">
                                <div class="dropdown">
                                    <select name="order_status" onchange="order_status(this.value)"
                                            ...
                                            data-id="{{$order['id']}}" id="order_status">
                                    ...
                                    <div class="media-body">
                                        <div class="row">
                                            ...
                                            <div class="col col-md-4">
                                                ...
                                                <input class="form-control" type="text" name="batch_number" value="{{$detail->batch_number}}" id="batch_number_{{$detail->id}}" required>
                                            </div>
                                            <div class="col col-md-4">
                                                <label for="expiry_date">Expiry Date</label>
                                                <input class="form-control" type="date" name="expiry_date" value="{{$detail->expiry_date}}" id="expiry_date_{{$detail->id}}" required>
                                            </div>
                                            <div class="col col-md-3 ">
                                                <br>
                                                <button class="form-control" onclick="submit_form({{$detail->id}})">Submit</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@push('script_2')
    <script>
        ...
        function order_status(status) {
            ...
            {
                $.get("{{route('admin.orders.check.bn.ed','')}}"+"/"+order_id, function(data) {
                    if(data != 0)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            Swal.fire({
                            title: '{{\App\CPU\translate('Are you sure Change this')}}?',
                            text: "{{\App\CPU\translate('You will not be able to revert this1')}}!",
                            showCancelButton: true,
                            confirmButtonColor: '#377dff',
                            cancelButtonColor: 'secondary',
                            confirmButtonText: '{{\App\CPU\translate('Yes, Change it')}}!'
        ...
        function package_detail_submit()
        {
            ...
            {
                var order_id='{{$order->id}}';
                $.post("{{route('admin.orders.store.package.detail','')}}"+"/"+order_id, $("#package_detail_form").serialize(), function(data) {
                    $('#pc_close').click();
                    Swal.fire({
                            title: '{{\App\CPU\translate('Are you sure Change this')}}?',
                            text: "{{\App\CPU\translate('You will not be able to revert this1')}}!",
    ...
    <script>
        function submit_form(id)
        {
            ...
            {
                $.get("{{route('admin.orders.update.bn.ed',['','',''])}}"+"/"+batch_number+"/"+expiry_date+"/"+id, function(data) {
