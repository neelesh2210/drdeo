...
<div class="tax1">
  	...
  	<header style="margin-top: 3%;">
		  ...
		  <p style="font-size: 11px;"><strong><b>Order ID:</b> </strong> {{ $order->id }}</p>
		  <p style="font-size: 11px;"><strong><b>Order Date:</b> </strong> {{date('d-m-Y H:i',strtotime($order['created_at']))}}</p>
		  <p style="font-size: 11px;"><strong><b>Invoice No:</b> </strong> {{ $order->id }}</p>
		  <p style="font-size: 11px;"><strong><b>Invoice Date:</b> </strong> {{date('d-m-Y')}}</p>
		  @php
		...
		@endphp
			<tr class="border">
			 ...
			 <h1> HSN Cod:  {{$details['product']?$details['product']->hsn:''}}	 </h1><br>
			  ...
			  <td style="text-align: center;font-size: 18px;"><h1>{{$details->qty}}</h1></td>
			  <td style="text-align: center;font-size: 18px;"><h1><b>{{\App\CPU\BackEndHelper::usd_to_currency($subtotal)}}</b></h1></td>
			  @if($tac_status == 1)
			  <td style="text-align: center;font-size: 18px;"><h1>CGST {{$details['product']->tax/2}}%</h1><br><h1>SGST {{$details['product']->tax/2}}%</h1></td>
			  <td style="text-align: center;font-size: 18px;"><h1>{{($subtotal/100)*$details['product']->tax}}</h1></td>
			  @else
			  <td style="text-align: center;font-size: 18px;"><h1>IGST {{$details['product']->tax}}%</h1></td>
			  <td style="text-align: center;font-size: 18px;"><h1>{{($subtotal/100)*$details['product']->tax}}</h1></td>
			  @endif
			  @php
